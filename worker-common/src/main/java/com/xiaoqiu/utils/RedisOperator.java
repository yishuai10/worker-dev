...
public class RedisOperator {
	...
	/**
	 * 实现命令：GET key，返回 key所关联的字符串值。 value
	 */
...
//		redis -> pipeline
	...
	/**
	 * 实现命令：LPUSH key value，将一个值 value插入到列表 key的表头 执行 LPUSH命令后，列表的长度。
	 */
	...
	/**
	 * 实现命令：LPOP key，移除并返回列表 key的头元素。 列表key的头元素。
	 */
	...
	/**
	 * 实现命令：RPUSH key value，将一个值 value插入到列表 key的表尾(最右边)。 执行 LPUSH命令后，列表的长度。
	 */
